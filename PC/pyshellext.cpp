...
CLASS_GUID) PyShellExt : public RuntimeClass <
                    ...
                    if (wlen) {
                        ...
                        if (MultiByteToWideChar(CP_ACP, 0, temp, (int) len, args, wlen + 1)) {
                            *pArguments = args;
                            ...
                            OutputDebugString(
                                    L"PyShellExt::GetArguments - failed to convert multi-byte to wide-char path");
                            CoTaskMemFree(args);
                            hr = E_FAIL;
                        }
                    } else {
...
STDAPI DllRegisterServer() {
    ...
    if (res == ERROR_ACCESS_DENIED) {
        OutputDebugString(
                L"PyShellExt::DllRegisterServer - failed to write per-machine registration. Attempting per-user instead.");
